...
struct TaskList: View {
    ...
    func showingTasks() -> [Task] {
        if showAll {
            return tvm.todayTasks
        }

        return tvm.todayTasks.filter { $0.done ? false : true }
    }
        ...
        VStack(spacing: 0) {
            HStack {
                HStack(spacing: 0) {
                    ...
                    todaysTasksText(text: "\(tvm.todayDoneTasks.count)")
                        .foregroundColor(pvm.status.MainColor())
                    todaysTasksText(text: "/\(tvm.todayTasks.count)")
                        .foregroundColor(Color("Black40"))
                }
